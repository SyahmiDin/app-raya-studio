...
export default function SalesReportPage() {
    ...
    async function fetchSalesData() {
        ...
        // 2. Ambil semua booking yang DAH DIBAYAR (Tambahan column 'pic' di sini)
      ...
      `,
        ...
        const validSales = (data || []).filter(
            (b) => !b.client_name?.toUpperCase().includes("ADMIN BLOCK"),
        );
        ...
        validSales.forEach((b) => {
            ...
            const monthYear = dateObj.toLocaleDateString("ms-MY", {
                month: "long",
                year: "numeric",
            });
        ...
        // --- FORMAT SENARAI PELANGGAN & KIRA PRESTASI STAFF/PIC ---
        const staffCounts = {}; 
        const picCounts = {}; 

        // --- PEMBOLEHUBAH BARU UNTUK UNTUNG BERSIH ---
        let totalStripeFees = 0;
        let totalAffiliateComms = 0;
        let totalPicComms = 0;

        const formattedClients = validSales
            .map((b) => {
                ...
                // KIRA CAJ STRIPE (3% dari harga + RM1)
                const stripeFee = (b.final_price || 0) * 0.03 + 1;
                totalStripeFees += stripeFee;

                // --- LOGIK AFFILIATE (STAFF REFERRAL) ---
                let staffName = "-";
                ...
                if (b.referral_code) {
                    ...
                    if (cleanCode !== "") {
                        ...
                        staffCommissionValue = (b.final_price || 0) * 0.1;
                        
                        if (staffName !== "-") {
                            // Masukkan ke dalam Total Keseluruhan Komisen Affiliate
                            totalAffiliateComms += staffCommissionValue;
                            ...
                            staffCounts[staffName].commission += staffCommissionValue;
                        }
                    }
                }

                // --- LOGIK PIC KOMISEN (10%) ---
                let picName = b.pic || "-";
                let picCommissionValue = 0;

                if (picName !== "-" && picName.trim() !== "") {
                    picName = picName.trim().toUpperCase(); 
                    picCommissionValue = (b.final_price || 0) * 0.1; 
                    
                    // Masukkan ke dalam Total Keseluruhan Komisen PIC
                    totalPicComms += picCommissionValue;

                    if (!picCounts[picName]) {
                        picCounts[picName] = { count: 0, revenue: 0, commission: 0 };
                    }
                    picCounts[picName].count += 1;
                    picCounts[picName].revenue += b.final_price || 0;
                    picCounts[picName].commission += picCommissionValue;
                }
        ...
        const allPics = Object.entries(picCounts)
            .map(([name, stats]) => ({ name, ...stats }))
            .sort((a, b) => b.count - a.count);

        // KIRA UNTUNG BERSIH
        const netProfitValue = revenue - totalStripeFees - totalAffiliateComms - totalPicComms;

        // Simpan ke State
        setStaffRanking(allStaffs);
        setPicRanking(allPics);
        setClientList(formattedClients);
        setProfitBreakdown({
            stripeFees: totalStripeFees,
            affiliateComms: totalAffiliateComms,
            picComms: totalPicComms,
        ...
        <div className="min-h-screen bg-slate-50 font-sans pb-12 print:bg-white print:pb-0">
                ...
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="text-white text-center md:text-left">
                        ...
                        <p className="text-indigo-200 mt-1 text-sm md:text-base">
                            Ringkasan pendapatan dan prestasi pakej Studio ABG
                            Raya.
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <a
                            href="/admin"
                            className="group flex justify-center items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-full backdrop-blur-sm transition-all text-sm font-medium border border-white/20"
                        >
                            <span>← Kembali</span>
                        </a>
                    </div>
            ...
            <div className="hidden print:block text-center py-6 border-b-2 border-gray-800 mb-6">
                <h1 className="text-3xl font-black uppercase tracking-widest">
                    Laporan Jualan Studio ABG Raya
                </h1>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in-up">
                            <div className="bg-white rounded-2xl shadow-lg p-6 border border-emerald-100 relative overflow-hidden print-clean">
                                <div className="absolute -right-4 -top-4 text-7xl opacity-5">
                                    💰
                                </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-lg p-6 border border-purple-100 relative overflow-hidden print-clean">
                                ...
                                <p className="text-[#412986] font-bold uppercase tracking-wider text-xs mb-2">
                                    Sasaran Jualan
                                </p>
                                        ...
                                        <table className="w-full text-left">
                                            <thead className="bg-gray-50/50 text-gray-400 text-xs uppercase tracking-wider">
                                                <tr>
                                                    ...
                                                    <th className="p-4 text-right pr-6 font-medium">
                                                        Nilai Jualan
                                                    </th>
                                                </tr>
                                            </thead>
                                                ...
                                                {monthlyStats.map(
                                                    (month, idx) => (
                                                        <tr
                                                            ...
                                                            className="hover:bg-gray-50 transition"
                                                        >
                                                            <td className="p-4 pl-6 font-bold text-gray-700 capitalize">
                                                                {month.month}
                                                            </td>
                                                            <td className="p-4 text-center text-gray-600 font-medium">
                                                                <span className="bg-gray-100 px-3 py-1 rounded-full text-xs">
                        ...
                        {/* --- 2.5 PENGIRAAN UNTUNG BERSIH (BARU) --- */}
                        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mt-6 animate-fade-in-up delay-150 print-clean">
                            ...
                            <div className="p-6">
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center md:text-left divide-y md:divide-y-0 md:divide-x divide-gray-100">
                                    
                                    <div className="p-2">
                                        <p className="text-gray-500 text-[10px] font-bold uppercase tracking-wider mb-1">Jualan Kasar</p>
                                    ...
                                    <div className="p-2">
                                        <p className="text-red-400 text-[10px] font-bold uppercase tracking-wider mb-1">- Caj Stripe</p>
                                        <p className="text-xl font-bold text-red-500">
                                            RM{profitBreakdown.stripeFees.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                                        </p>
                                        <p className="text-[10px] text-gray-400 mt-1">3% + RM1 / transaksi</p>
                                    </div>
                                    
                                    <div className="p-2">
                                        <p className="text-orange-400 text-[10px] font-bold uppercase tracking-wider mb-1">- Komisen Affiliate</p>
                                        <p className="text-xl font-bold text-orange-500">
                                            RM{profitBreakdown.affiliateComms.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                                        </p>
                                    </div>
                                    
                                    <div className="p-2">
                                        <p className="text-blue-400 text-[10px] font-bold uppercase tracking-wider mb-1">- Komisen PIC</p>
                                        <p className="text-xl font-bold text-blue-500">
                                            RM{profitBreakdown.picComms.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                                        </p>
                                    </div>
                                    
                                    <div className="p-2 md:bg-emerald-50 md:px-4 rounded-xl flex flex-col justify-center">
                                        <p className="text-emerald-600 text-[10px] font-bold uppercase tracking-wider mb-1">Untung Bersih</p>
                                        <p className="text-2xl font-black text-emerald-600">
                                            RM{profitBreakdown.netProfit.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                                        </p>
                                    </div>
                        ...
                        {/* --- 3. SENARAI PELANGGAN BERJAYA DENGAN STAFF REFERRAL & PIC --- */}
                        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mt-6 animate-fade-in-up delay-200 print-clean">
                            ...
                            <div className="overflow-x-auto p-0">
                                {clientList.length === 0 ? (
                                    <p className="p-8 text-center text-gray-400">
                                        Tiada rekod tempahan pelanggan.
                                    </p>
                                ) : (
                                ...
                                <div className="overflow-x-auto p-0">
                                    <table className="w-full text-left min-w-[600px]">
                                        <thead className="bg-white text-gray-400 text-xs uppercase tracking-wider border-b border-gray-100">
                                            <tr>
                                                <th className="p-4 pl-6 font-medium">
                                                    Kedudukan & Nama Affiliate
                                                </th>
                                                <th className="p-4 text-center font-medium">
                                                    Jumlah Pelanggan
                                                </th>
                                                <th className="p-4 text-right pr-6 font-medium">
                                                    Total Komisen Terkumpul
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-50">
                                                        ...
                                                        >
                                                            <td className="p-4 pl-6">
                                                                <div className="flex items-center gap-3">
                                                                    <span className="text-2xl w-8 text-center flex justify-center">
                                                                        {
                                                                            rankIcon
                                                                        }
                                                                    </span>
                        ...
                        {/* --- 5. SENARAI KOMISEN PIC (BARU) --- */}
                        {picRanking.length > 0 && (
                            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mt-6 animate-fade-in-up delay-[400ms] print-clean">
                                <div className="bg-gray-50 p-5 border-b border-gray-100 flex justify-between items-center">
                                    <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2">
                                        Komisen PIC
                                    </h2>
